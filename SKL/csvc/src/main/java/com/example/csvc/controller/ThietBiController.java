...
//    }
    ...
    public String forwardThietBi(@RequestParam (value = "idForwardList[]") Integer[] idForwardList, Integer idvitri){
        ...
        String tenCoSoMoi=viTri.getKhu().getCoSo().getTenCoso();
        String tenKhuMoi=viTri.getKhu().getTenKhu();
        String tenViTriMoi=viTri.getTenViTri();

        for (Integer idForward:idForwardList){
           ThietBi thietBi=thietBiService.findThietBiById(idForward);

           String tenCoSoCu=thietBi.getViTri().getKhu().getCoSo().getTenCoso();
           String tenKhuCu=thietBi.getViTri().getKhu().getTenKhu();
           String tenviTriCu=thietBi.getViTri().getTenViTri();

           thietBi.setViTri(viTri);
           thietBiService.saveThietBi(thietBi);
            DieuChuyen dieuChuyen=new DieuChuyen(thietBi,tenCoSoCu,tenKhuCu,tenviTriCu,tenCoSoMoi,tenKhuMoi,tenViTriMoi,LocalDate.now());
           dieuChuyenService.saveDieuChuyen(dieuChuyen);
        }
