...
function deleteCustomer(index) {
    var isDelete = confirm('Ban co muon xoa '+listCustommer[index].name+'?')
    if (isDelete){
...
function updateCustomer(index) {
    ...
    if (index == '') {
        alert('Bạn đang ở chế độ Thêm mới')
    } else {
        var isAddCustomer = (checkFullName() && checkCMND() && checkBirthDay() && checkEmail() && checkAddress() && checkRentDays() && checkAttach() && checkDiscount())
...
function viewInfor(index) {
    document.getElementById('detailinfor').style.display = "block";
    ...
    document.getElementById('totalpay').innerHTML = listCustommer[index].getName() + "&nbsp;" + total + ' USD';
}

function closeForm() {
    document.getElementById('detailinfor').style.display = "none";
}
...
function checkBirthDay() {
    ...
    if (birthDay.trim().length === 0) {
        ...
        if (birthDay.substring(0, 2) > 31) {
            ...
            return false
        } else if ((birthDay.substring(6, 11) > 2021) || (birthDay.substring(6, 11) < 1920)) {
...
function checkEmail() {
    ...
    if (email.length === 0) {
        ...
        document.getElementById('erroremail').innerHTML = "Nhập lại email đúng dịnh dạng";
        return false
    } else if (posa > posdot) {
        document.getElementById('erroremail').innerHTML = "Nhập lại email đúng dịnh dạng";
        return false
